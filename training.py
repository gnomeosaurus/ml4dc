...
datas[2800] = runIDs   #Append runID back after scaling
datas[2801] = lumiIDs  #Append lumiID back after scaling
...
def add_flags_from_json_PD(output_json, X_test):
    ...
    for i in range(0, X_test.shape[0]):
        orig_runid = X_test[2800][i+last_train_idx]
        # orig_runid = int(orig_runid)
        orig_lumid = X_test[2801][i+last_train_idx]
        # orig_lumid = int(orig_lumid)
        new_json_class[i,0] = int(json_checker(output_json, orig_runid, orig_lumid))
...
X_test = X_test.drop(['qlabel'], axis=1)
X_test = X_test.drop(['PD_json'], axis=1)


X_train.drop(2800, axis=1, inplace=True) #drop RunID before training
X_train.drop(2801, axis=1, inplace=True) #drop LumiID before training
X_test.drop(2800, axis=1, inplace=True) #drop RunID before training
X_test.drop(2801, axis=1, inplace=True) #drop LumiID before training
...
checkpointer = ModelCheckpoint(filepath="3pds_model.h5",
                               verbose=0,
                               save_best_only=True)
